...
$full_name = htmlspecialchars($_SESSION['full_name'] ?? 'Accreditor');
...
if ($repo_tables_ready) {
    if (strpos($role, 'admin') !== false) {
        ...
        $stmt = $conn->prepare("SELECT r.*, p.program_code,
                                ...
                                LEFT JOIN programs p ON r.program_id = p.program_id
                                WHERE rm.user_id = ? AND rm.member_role = 'accreditor' AND rm.is_active = 1
                                ORDER BY r.created_at DESC");
...
<body class="min-h-screen bg-slate-50 text-slate-800">
  <header class="topbar">
    <div class="left">
      <div class="brand">
        <i class="fas fa-user-check"></i> <span>Accreditor Repository Review</span>
      </div>
    </div>
    <div class="right">
      <div class="user-profile">
        <div class="user-info hidden sm:block">
          ...
          <span class="user-role"><?= htmlspecialchars($_SESSION['role'] ?? 'Accreditor') ?></span>
        </div>
  ...
  <main class="max-w-7xl mx-auto px-4 py-24">
    <div class="mb-10 text-center">
      <h1 class="text-3xl font-bold text-slate-800">Repository Review Access</h1>
      <p class="text-slate-500 mt-2">Review only the repositories assigned to you and leave comments on uploaded accreditation files.</p>
    </div>
    ...
    <?php elseif (empty($repositories)): ?>
      <div class="max-w-3xl mx-auto bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-500 shadow-sm">
        ...
        <p class="mt-2">Once the admin grants repository access, your review work will appear here.</p>
      </div>
    <?php else: ?>
        ...
        <?php foreach ($repositories as $repository): ?>
          <a href="repository_workspace.php?repository_id=<?= (int) $repository['repository_id'] ?>" class="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all p-6">
            <div class="flex items-start justify-between gap-4">
              <div class="w-14 h-14 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-2xl group-hover:bg-emerald-600 group-hover:text-white transition-colors">
            ...
            <h2 class="mt-5 text-xl font-bold text-slate-800 group-hover:text-emerald-700 transition-colors"><?= htmlspecialchars($repository['repository_name']) ?></h2>
            ...
            <div class="mt-6 text-sm font-semibold text-emerald-600 group-hover:translate-x-1 transition-transform">
              Start Review <i class="fas fa-arrow-right ml-1"></i>
            </div>
          </a>
        <?php endforeach; ?>
      </div>
